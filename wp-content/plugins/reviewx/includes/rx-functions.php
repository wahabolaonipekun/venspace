...
function reviewx_filter_review() {
    ...
    $paginaiton     =  isset( $_POST[ 'rx_pagination' ] ) ? sanitize_text_field( $_POST[ 'rx_pagination' ] ): '';
    ...
    $post_title     = isset( $_POST[ 'rx_post_title' ] ) ? sanitize_text_field( $_POST[ 'rx_post_title' ] ) : '';
...
function rx_touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0, $comment_date = null ) {
	...
	echo '</div><input type="hidden" id="ss" name="ss" value="' . sanitize_text_field($ss) . '" />';
	...
	foreach ( $map as $timeunit => $value ) {
		...
		echo '<input type="hidden" id="hidden_' . $timeunit . '" name="hidden_' . $timeunit . '" value="' . sanitize_text_field($unit) . '" />' . "\n";
		$cur_timeunit = 'cur_' . $timeunit;
		echo '<input type="hidden" id="' . $cur_timeunit . '" name="' . $cur_timeunit . '" value="' . sanitize_text_field($curr) . '" />' . "\n";
	}
	?>
