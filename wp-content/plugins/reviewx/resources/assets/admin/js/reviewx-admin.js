(function ($) {
	...
	*/
	...
	/* -----------------------------------
			Google review admin js start here
		-----------------------------------
	*/
	const rxGoogleReviewFrom = $(".rx-google-review");
	rxGoogleReviewFrom.on("submit", (e) => {
		e.preventDefault();
		
		// var googleInputObj = {};
		// $("#rx-google-review input[type='text']").each(function () {
		// 	googleInputObj[$(this).attr("name")] = $(this).val();
			
		// });
		var googleInputObj = {
			rx_review_id: $("#google-review-location-id").val(),
			locationApi: $("#google-review-location-api").val(),
		  };
		  console.log(googleInputObj)
		// googleInputObj['rx_review_api'] = document.getElementById('google-review-location-id').value
		// googleInputObj['rx_review_id'] = document.getElementById('google-review-location-api').value
		// alert(googleInputObj['rx_review_api'])
		$.ajax({
			type: 'post',
			url: ajaxurl,
			data: {
				action: 'rx_google_review_from',
				rx_google_review: googleInputObj
