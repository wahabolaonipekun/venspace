...
class Editor extends Controller  
{
     ...
     * @return string
     */
    public function setProductId($orderItems)
    {
        foreach ($orderItems as $orderItem) {
            $product = get_post($orderItem);                      
            $this->productId = $this->orderItem[$product->ID] = $orderItem;
        }
    }
    ...
    {        
        $image = wp_get_attachment_image_src( get_post_thumbnail_id( $this->productId ), 'thumbnail' );
        $imageTag ="{$image[0]}";
        return [ 'type' => 'product_image', 'url' => $imageTag];
    }
}
